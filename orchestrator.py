    ...
    def _dispatch_provider(p):
        ...
        if p.kind == "ollama":
            ...
            if p.pool == "local":
                ...
                # Toolbox model: pick the right tool group for this request.
                # 32 full tool schemas ≈ 6400 tokens (overflows 8192 CTX).
                # select_local_tools() returns 4-8 tools for the task type
                # (~800-1200 tokens), leaving room for real output.
                # CLAF_LOCAL_MAX_TOOLS=0 → strip tools (explicit no-tools mode).
                # Default is 6 — enough for one tool group.
                _tools_eff = select_local_tools(body, _tools) if _tools else _tools
                if _tools and _tools_eff != _tools:
            ...
            try:
                ...
                if provider is None:
                    ...
                    if _local is not None:
                        # If CLAF_LOCAL_MAX_TOOLS=0, local can't run tools at all.
                        # Return a visible message rather than a broken prose reply.
                        ...
                        if _has_tools and _local_max_tools == 0:
                            ...
                            _rl_msg = (
                                "[CLAF: all cloud peers rate-limited — local model is in "
                                "no-tools mode (CLAF_LOCAL_MAX_TOOLS=0). Set it to 6 in "
                                ".env to enable local tool use, or wait 30s and retry.]"
                            )
