...
ROUTING_ORACLE: dict[str, str | None] = {
    ...
    "p3_web_search":      "local" if APOCALYPTIC else "cloud",  # web search = cloud for accuracy/currency, local only off-grid
...
BROWSER_TOOLS = [
    ...
    _tool("mcp__sensei__fill", {"where": {"type": "string"}, "text": {"type": "string"}}, ["where", "text"], "fill field"),
...
FS_TOOLS = [
    ...
    _tool("Write", {"file_path": {"type": "string"}, "content": {"type": "string"}}, ["file_path", "content"], "write a file"),
    _tool("Edit", {"file_path": {"type": "string"}, "old_string": {"type": "string"}, "new_string": {"type": "string"}}, ["file_path", "old_string", "new_string"], "edit a file"),
    _tool("Grep", {"pattern": {"type": "string"}}, ["pattern"], "search file contents"),
]
TASK_TOOLS = [
    _tool("TaskList", {}, [], "list tasks"),
    _tool("TaskCreate", {"subject": {"type": "string"}, "description": {"type": "string"}}, ["subject", "description"], "create a task"),
]
ALL_TOOLS = BROWSER_TOOLS + FS_TOOLS + TASK_TOOLS

SYSTEM = ("You are Claude Code. Identity: MCP. Act through tools, show evidence, "
          "never narrate a plan, never say you can't.") * 1  # small stand-in system

GIVEUP = ("i cannot", "unable to", "i'll stop", "please try again",
          "i can't", "cannot access", "unable to connect", "is unavailable")
...
HOOK_PREAMBLE = ("[STANDING ORDERS] " + ("browser console_logs network_requests read_full "
                 "identify page type pick tool one action screenshot confirm. " * 60))

def _user(text): return {"role": "user", "content": text}
...
CORPUS = [
    {"id": "p1_open_tab", "layer": "ROUTING+BEHAVIOR", "tools": ALL_TOOLS,
     "messages": [_user("open an mcp tab to kimi.com")],
     "expect_tool": "mcp__sensei__tab_create", "forbid": GIVEUP + PLAN_PREAMBLE},
    {"id": "p2_loop_continue", "layer": "CAPABILITY", "tools": ALL_TOOLS,
     "messages": [_user("open a tab to example.com then screenshot it"),
                  _assist_tooluse("mcp__sensei__tab_create", {"url": "https://example.com"}),
                  _tool_result("toolu_mcp__sensei__tab_create", "tab opened, id=2")],
     "expect_tool": "mcp__sensei__screenshot", "forbid": GIVEUP},
    {"id": "p3_web_search", "layer": "CAPABILITY", "tools": ALL_TOOLS,
     "messages": [_user("search the web for the qwen3.5 context window size")],
     "expect_tool": "mcp__sensei__search", "forbid": GIVEUP},
    {"id": "p4_analyze_tradeoff", "layer": "ROUTING", "tools": ALL_TOOLS,
     "messages": [_user("analyze the trade-offs between QLoRA and full fine-tuning for a 9b model")],
     "expect_tool": None, "forbid": ()},
    {"id": "p5_write_memory", "layer": "CAPABILITY", "tools": ALL_TOOLS,
     "messages": [_user("write a file ~/notes.txt that says the parity test passed")],
     "expect_tool": "Write", "forbid": GIVEUP + PLAN_PREAMBLE},
    {"id": "p6_file_and_web", "layer": "CAPABILITY", "tools": ALL_TOOLS,
     ...
     "expect_tool": None,
     "expect_any_groups": [["Write", "Bash", "Edit"], ["mcp__sensei__tab_create", "mcp__sensei__browse"]],
     "forbid": GIVEUP},
    {"id": "p7_buried_command", "layer": "CONTEXT", "tools": ALL_TOOLS,
     "messages": [_user(HOOK_PREAMBLE + "\n\nACTUAL COMMAND: take a screenshot of the current page")],
     "expect_tool": "mcp__sensei__screenshot", "forbid": GIVEUP},
    {"id": "p8_error_retry", "layer": "BEHAVIOR", "tools": ALL_TOOLS,
     "messages": [_user("click the Submit button"),
     ...
     "expect_tool": None, "forbid": GIVEUP + ("i'll stop here",)},
    {"id": "p9_loop_at_cap", "layer": "TERMINATION", "tools": ALL_TOOLS,
     "messages": _loop_history(21),
     "expect_log": "loop_replan_injected", "expect_tool": None, "forbid": ()},
]
...
PARSER_CASES = [
    ...
    ("function_xml", '<function=tab_create>{"url": "https://kimi.com"}</function>', "mcp__sensei__tab_create", True),
    ("directive_bash", "BASH: ls -la", "Bash", True),
    ("bare_kv_NO_PARENS", "tab_create url=https://kimi.com", "mcp__sensei__tab_create", True),  # the known gap
]
...
def run_live():
    ...
    for c in CORPUS:
        ...
        body = {"model": "claude-sonnet-4-6", "max_tokens": 512,
                "system": SYSTEM, "messages": c["messages"], "tools": c.get("tools")}
        t0 = time.time()
        err = None
        try:
            resp = post(body)
        ...
        if c.get("expect_any_groups"):
            scores["CAPABILITY"] = all(any(g in tools for g in grp) for grp in c["expect_any_groups"])
        ...
        results.append({"id": c["id"], "layer": c["layer"], "route": actual_route,
                        "oracle": oracle, "tools": tools, "dt": round(dt, 1),
                        "err": err, "scores": scores,
                        "text_head": text[:80]})
    return results
...
def run_parser():
    ...
    for label, text, expect_name, should_parse in PARSER_CASES:
        ...
        rows.append({"case": label, "parsed_tool_use": used, "got": got,
                     "expected": expect_name, "PASS": ok})
    return rows
...
def main():
    ...
    for r in live:
        ...
        print(f"{r['id']:22} {r['layer']:16} {r['route']:6} {str(r['oracle']):6} "
              f"{r['dt']:>5}  {sc}{flag}")
        print(f"   tools={r['tools']}  emitted='{r['text_head']}'")
    print("\nPARSER UNIT (prompt 10)")
    for row in run_parser():
        print(f"  [{'PASS' if row['PASS'] else 'FAIL'}] {row['case']:20} "
