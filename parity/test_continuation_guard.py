...
def _():
    ...
    pushed = False
    should = (
        not tool_use and has_tools and not overflow and not pushed and _task_pending_count() > 0
    )
    assert should
...
def _():
    items = [
        {"id": i, "task": f"s{i}", "status": "done" if i < 999 else "pending"} for i in range(1000)
    ]
